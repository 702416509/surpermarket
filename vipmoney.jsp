...
<html lang="zh-CN">
  ...
  <c:if test="${sessionScope.user eq null}"><c:redirect url="login.jsp"/></c:if>
	...
	<c:if test="${ sessionScope.user.rolesClass.RId ne 3}">
		<div class="hdp-welcome col-md-offset-3 col-md-6" id="hdp-welcome"><span>权限不足!</span></div>
	</c:if>
	<c:if test="${ sessionScope.user.rolesClass.RId eq 3}">
	<div class="hdp-body" id="hdp-body">
		<form class="form-horizontal" method="post" action="dataaddServlet?action=addvipmoneynow">
		  <div class="form-group">
		    <label for="hdp-VId" class="col-sm-2 control-label">会员卡号</label>
		    <div class="col-sm-10">
		      <input type="text" class="form-control" name="hdp-VId" placeholder="会员卡号">
		    </div>
		  </div>
		  <div class="form-group">
		    <label for="hdp-Vipmoney" class="col-sm-2 control-label">充值金额</label>
		    <div class="col-sm-10">
				<input type="text" class="form-control" name="hdp-Vipmoney" placeholder="充值金额">
		    </div>
		  </div>
		  <div class="form-group">
